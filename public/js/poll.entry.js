...
/***/ function(module, exports, __webpack_require__) {
	...
	var React = __webpack_require__(1),
	    ...
	    api = __webpack_require__(308);

	var PollingBooth = React.createClass({
		displayName: 'PollingBooth',
		...
		componentDidMount: function componentDidMount() {
			...
			req.setRequestHeader('Authorization', 'Token ' + JSON.parse(localStorage.getItem('tokens'))[this.state.pollId]);
			...
			if (this.state.poll) {
				if (Date.parse(this.state.poll.startDate) > new Date()) {
					...
					return React.createElement(
						...
						React.createElement(PollResults, { pollId: this.state.pollId })
					);
				} else {
					return React.createElement(
						'div',
	...
	var PollResults = React.createClass({
		...
		componentDidMount: function componentDidMount() {
			...
			req.setRequestHeader('Authorization', 'Token ' + JSON.parse(localStorage.getItem('tokens'))[this.props.pollId]);
			...
			if (this.state.results) {
				return React.createElement(
					...
					React.createElement(
						...
						React.createElement(
							'a',
							{ href: 'javascript:void(0)', onClick: this.previous, className: 'pull-left' },
							'Previous'
						),
						React.createElement(
							'a',
							{ href: 'javascript:void(0)', onClick: this.next, className: 'pull-right' },
							'Next'
						)
					),
					React.createElement(
						'div',
						{ className: 'row', style: { border: "solid 1px grey", height: "25em", padding: "1em" } },
						React.createElement(ResultStep, { results: this.state.results[this.state.resultIndex] })
					)
				);
			}
			return React.createElement(
				...
				React.createElement(
					'h4',
					{ className: 'text-center', style: { marginTop: "4em" } },
			...
			return React.createElement(
				...
				React.createElement(
					...
					React.createElement(
						'b',
						null,
						new Date(this.props.startDate).toString()
					)
				)
			);
		}
		...
		render: function render() {
			return React.createElement(
				...
				React.createElement(
					'div',
					{ className: 'col-sm-6' },
					React.createElement(Ballot, { pollId: this.props.pollId })
				),
				React.createElement(
					'div',
					{ className: 'col-sm-6' },
		...
		componentDidMount: function componentDidMount() {
			...
			if (existing) {
				...
				if (ballots[this.props.pollId]) {
					...
					ballotRequest.setRequestHeader('Authorization', 'Token ' + JSON.parse(localStorage.getItem('tokens'))[this.props.pollId]);
	...
	ReactDOM.render(React.createElement(PollingBooth, null), document.getElementById('content'));
